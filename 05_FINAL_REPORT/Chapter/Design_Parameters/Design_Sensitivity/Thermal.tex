...
In order to analyse layup mass performance and changes due to variable atmospheric conditions a heat flux sensitivity is performed. The heat flux of a possible trajectory is multiplied by scaling factors to find relative values. The trajectory is found using a diameter of 12 $[m]$ and the relative factors are 0.25, 0.5, 0.75, 0.9, 1.0, 1.1, 1.4, 1.5, 2.0, 2.5 and 3.0. The results are shown in Figure \ref{fig:sensiticvityq}. The horizontal axis show the relative factors and the areal density is shown on the vertical axis. 
