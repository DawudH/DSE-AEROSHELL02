...
It is essential that the diameter of centerbody and crew module is slightly smaller than the maximum 5 [$m$] diameter to account for inflatable stowage. To this end, the centerbody diameter of 4.5 [$m$] rather than 5 [$m$] is imposed. In addition, this smaller diameter accounts for launcher fairing considerations.
...
Deployment is schematically summarized in Figure \ref{fig:deplflow}. Detachment is followed upon by inflation, which gives the inflated system the shape defined in subsection \ref{subsec:infldes}. Inner bladder volumes are inflated first, to provide more stiffness towards the root where loads are highest and provide a stiff basis for the outer bladder volumes to inflate.
