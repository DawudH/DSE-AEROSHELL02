...
Using a temperature of 202 [$K$] inside the inflatable yields a total gas mass of ??? [$kg$] that is to be held within the pressure tank. An additional contingency of 50 $\%$. This contingency is so large because leakage requires additional gas to make up for the lost volume, so called make up gas \cite{Jenkins2001}.

An initial estimate for the tank pressure and temperature are 27.6 [$MPa$] and 323 [$K$] \cite[p.545]{Wertz2011}. Based on the required pressure and a temperature of 202 [$K$] in the inflatable, by use of the ideal gas law, a total pressure tank volume of ... [$m^{3}$] follows. Corresponding to this tank volume is a required tank mass, dependent on the material used. Composite overwrapped pressure vessels offer significant mass advantages with respect to metallic tanks and therefore such a pressure vessel is selected. Tank mass is estimated from empirical relations established by Zakrwski \cite[p.546]{Wertz2011} to be … [$kg$] for the selected tank pressure and volume via Equation \ref{eq:tmass}.
\begin{equation}
...
This mass estimate is consistent with the estimate using the parametric mass model described in section \ref{subsec:structool}, where inflation system mass is estimated to be … $[kg]$. Including a contingency of $25\%$ for valves and feed lines yields a total inflation system mass of … $[kg]$. 
