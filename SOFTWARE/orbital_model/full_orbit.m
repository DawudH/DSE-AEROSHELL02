function [R,V,a,conditions] = full_orbit(rx,CD)
constants
input_var
conditions.atmos = false; conditions.crash = false; conditions.orbit = false; 
R(1,:) = [rx,ry,0]; %[m]
V(1,:) = [0,-v,0]; %[m/s]
a(1,:) = [0,0,0]; %[m/s^2]
CL = CLCD*CD; %[-]
new = orbit(R,V,a,CD,CL,dt,R_m,Omega_m,S,m);
...
while true
    ...
    new = orbit(new.R,new.V,new.a,CD,CL,dt,R_m,Omega_m,S,m);
    R(i+1,:) = new.R;
    V(i+1,:) = new.V;
    a(i+1,:) = new.a;
    if new.R(1)>0
        conditions.orbit = true;
    end
    %check for being in the atmosphere
    if norm(new.R)<(R_m+h_atm)
        conditions.atmos = true;
        %check for crash
        if norm(new.R)<R_m && (conditions.orbit == false)
    ...
    if (new.R(2)<-R_m) && (conditions.atmos == false) && (v>V_esc0)
        conditions.orbit = false;
        break;
    end
