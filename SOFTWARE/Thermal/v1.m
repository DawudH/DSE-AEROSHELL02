...
alpha=k(1)/rho(1)/cp(1);tau=totalt*dt;k=k(1);x=dx*(imax-1);q0=q(1);
usssana = q0/k*sqrt(pi*alpha*tau);
usssnum = T(1,end)-T(1,1);
ussbana = 2*q0*sqrt(alpha*tau/pi)/k*exp(-x^2/4/alpha/tau)-q0*x/k*erfc(x/2/sqrt(alpha*tau));
