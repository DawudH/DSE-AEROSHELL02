...
T1 = T;
t1 = t;
qmax_array1 = qmax_array;
clear('T','t','qmax-array')
load('./DesignData/entry_rho_1_1.mat','T','t','qmax_array','A_whetted')
T3 = T;
t3 = t;
qmax_array3 = qmax_array;
clear('T','t','qmax-array')
% introduce orbit
t_orbit     = 24.6*60*60;
t2          = round(10*(t1(end)+0.1:0.1:(t_orbit*0.1)+t1(end)-0.1))/10;
T2          = ones(length(t2),1)*3;
qmax_array2 = zeros(length(t2),1);
qmax_array2(end) = 0.1;
%combine
t3 = t3 + t2(end)+0.1;
t = [t1 t2];% t3];
T = [T1' T2'];% T3'];
qmax_array = [qmax_array1 qmax_array2'];% qmax_array3];
...
for n=1:nmax-1
    ...
    qrb = -emiss*sig*(T(end,n)^4-0^4); % Radiation back
