...
% Aero input, qsdot
load('heatflux_orbit_iteration_1_1.mat','T','t','qmax_array','A_whetted')
...
% Now optimize! :D
  ...
  if results(1) > Tallow(1)
     X = 'No Go! Use a different layup';
     disp(X)
  else      
     for z = 1:length(L)-1
        if results(z+1) > Tallow(z+1)
            while results(z+1) > Tallow(z+1)  %Nextel(1643 623 473) harnessedsatin(2073 1373 623 473)
            ...
            disp(['hij dacht dat laag ' num2str(z+1) ' te warm werd' ])
        elseif (results(z+1) < Tallow(z+1) && (layup(1,1) >= 0.508/1000))
            while (results(z+1) < Tallow(z+1) && (layup(1,1) > 0.508/1000))  %Nextel(1643 623 473) harnessedsatin(2073 1373 623 473)
                layup(z,1) =  layup(z,1) - 1.0*2.54e-5;
            ...
            disp(['hij dacht dat laag ' num2str(z+1) ' te koud werd' ])
        end
            disp(output)  
  ...
  format long
  disp('the optimum thicknisses are: ')
  disp(L)