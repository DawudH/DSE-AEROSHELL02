...
% Aero input, qsdot
load('heatflux_out_d12_just_orbit.mat','T','t','qmax_array')
tq = find(not(qmax_array==0));

% Change the heatflux to a triangle that can be changed in size
timefactor = 2.0;
fluxfactor = 1.0;
qaerotool  = fluxfactor*qmax_array(tq(1):tq(end));
qaero      = zeros(1,length(qaerotool));
qaero(1000:2192) = linspace(0,(1/timefactor)*max(qaerotool),2192-1000+1);
qaero(2192:4500) = linspace((1/timefactor)*max(qaerotool),0,4500-2192+1);

Tatm  = T(tq(1):tq(end)); 
timeq = timefactor*t(tq(1):tq(end))-t(tq(1));
t_or = t;
clear('T','t')
...
% layup(2*i-1:2*i,1) =  layup(2*i-1:2*i,1) + 1.0*2.54e-5;
     ...
     L = layup(:,1);
        ...
        dt   = timefactor*0.1;    % time step, chooseable
        T0 = 293;
        q0 = qaero*10000;
