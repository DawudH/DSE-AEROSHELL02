...
Applying the empirical relations with a design entry mass of $\gls{sym:m0} = 10000$ [$kg$] yields the relations as shown in Figure \ref{fig:rigid}.
...
Figure \ref{fig:rigid} provides an estimate of the sensitivity of decelerator mass with regard to trajectory, reflected by peak dynamic pressure and heat load. This allows for a first order estimate of the mass of the rigid concept.
...
The mass is evaluated at a heat load of 4150 [$\frac{J}{cm^2}$], corresponding to output of the thermal analysis tool in Chapter of \ref{ch:thermtool} by integrating heat flux computed by the aerodynamic tool for the reference trajectory, which is considered a relatively low value keeping in mind that a single orbit is shared by the rigid and inflatable concepts at this point. This is combined with a peak dynamic pressure of 2400 [Pa] following the selected orbit from Chapter \ref{ch:astrocontrol} yields the mass estimate as in Table \ref{tab:strucmassrigid}. Note that the mass estimate for the backshell also included part of the payload structure which should be taken into account. The conclusions in this chapter regarding the total mass do however remain valid. 
