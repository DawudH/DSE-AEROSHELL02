...
\item{Trajectory}: All concepts under analysis are asumed to have the same trajectory.
\end{itemize}
...
\label{eq:therm5}
\end{equation}
Figure \ref{fig:thermalval} shows the temperature of a semi-infinite copper block, which is the same Smith uses to verify his model \cite{Smith2011}. Semi-infinite in this case means that the block only has a finite depth. In this case the constant applied heat flux is $30 \frac{W}{cm^2}$, the initial temperature is $20 ^{\circ}C$ and the time at which the temperature is reported is $120 [s]$. Using Figure \ref{fig:thermalval} to compare this analytical solution with the numerical tool shows that the tool does not function correctly. It is not able to give a correct time response, which is essential for the implementation of the tool within the design of the aeroshell. The temperature increases faster in the numerical model than the analytical solution prescribes.
